...
pub struct UserDataKey<T> {
    pub(crate) key: sys::hb_user_data_key_t,
...
impl<T> UserDataKey<T> {
    ...
    pub const fn new() -> Self {
        Self {
            key: sys::hb_user_data_key_t { unused: 0 },
            _marker: PhantomData,
        }
    }
}
...
macro_rules! user_data_methods {
    ($ffi_get_user_data: path, $ffi_set_user_data: path,) => {
        ...
        pub fn set_user_data<T: 'static>(
            ...
            unsafe extern "C" fn destructor<T>(ptr: *mut std::os::raw::c_void) {
            ...
            let ptr = ptr as *mut T as *mut std::os::raw::c_void;
            let status = unsafe {
                let replace = replace.then(|| 1).unwrap_or(0);
                $ffi_set_user_data(
                    self.as_mut_ptr(),
        ...
        pub fn remove_user_data<T: 'static>(
            ...
            let status = unsafe {
                $ffi_set_user_data(self.to_raw_none(), &mut key.key, std::ptr::null_mut(), None)
            };
            status.then(|| Ok(())).ok_or(crate::Error::NoMemory)
        }
        */
    };
}
